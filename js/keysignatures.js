class KeySignature {
  constructor(keySignature) {
    ...
    this.equivalentMajorKeySignature = this.getEquivalentMajorKeySignature(
      this.keySignature
    );
  }
  ...
  generateKeySignature(keySignature = null) {
    switch (keySignature) {
      case "minor":
        return this.minorkeysignatures[
          Math.floor(Math.random() * this.minorkeysignatures.length)
        ];
      case "major":
        return this.majorkeysignatures[
          Math.floor(Math.random() * this.majorkeysignatures.length)
        ];
      case "random":
        return this.keysignatures[
          Math.floor(Math.random() * this.keysignatures.length)
        ];
      case null:
        return "C";
      default:
  ...
  noteInKeySignature(note, keySignature = this.keySignature) {
    ...
    if (note.noteWithoutOctave in changedNotes) {
      ...
      if (keySignature == "C#") {
        if (note.noteWithoutOctave == "B") {
          changedNoteOctave += 1;
        }
      }

      changedNoteWithoutOctave = note.noteWithoutOctave.replace(
